...
def parameter_server(rank, size, num_batches, device, logger: Logger, collector: DataCollector, backend: str):
    ...
    for batch_idx in range(num_batches):
        ...
        batch_metrics['broadcast_params_time'] = broadcast_end - broadcast_start
        logger.log(f"[Parameter Server] Broadcasted parameters to workers.")
        ...
        reduce_grads_start = time.time()
        dist.reduce(tensor=grad_flat, dst=0, op=dist.ReduceOp.SUM)
        reduce_grads_end = time.time()
        batch_metrics['reduce_gradients_time'] = reduce_grads_end - reduce_grads_start
        logger.log(f"[Parameter Server] Reduced gradients from workers.")
...
def worker(rank, size, num_batches, batch_size, device, logger: Logger, collector: DataCollector, backend: str):
    ...
    for batch_idx in range(num_batches):
        ...
        broadcast_end = time.time()
        batch_metrics['broadcast_params_time'] = broadcast_end - broadcast_start
        logger.log(f"[Worker {rank}] Received broadcasted parameters from parameter server.")
        ...
        reduce_grads_end = time.time()
        batch_metrics['reduce_gradients_time'] = reduce_grads_end - reduce_grads_start
        logger.log(f"[Worker {rank}] Reduced gradients to parameter server.")
...
def plot_metrics(csv_filename, logger: Logger):
    ...
    ps_metrics = ['broadcast_params_time', 'reduce_gradients_time',
    ...
    worker_metrics = ['broadcast_params_time', 'unflatten_params_time', 'data_loading_time',
                      'data_to_device_time', 'zero_grad_time', 'forward_pass_time',
                      'compute_loss_time', 'backward_pass_time', 'flatten_grad_time',
                      'reduce_gradients_time', 'batch_duration', 'loss', 'throughput_samples_per_sec',
                      'cpu_percent', 'memory_total', 'memory_available', 'memory_used', 'memory_percent']
    ...
    aggregated_metrics = ['broadcast_params_time', 'unflatten_params_time', 'data_loading_time',
                          'data_to_device_time', 'zero_grad_time', 'forward_pass_time',
                          'compute_loss_time', 'backward_pass_time', 'flatten_grad_time',
                          'reduce_gradients_time', 'batch_duration', 'loss', 'throughput_samples_per_sec',
                          'cpu_percent', 'memory_total', 'memory_available', 'memory_used', 'memory_percent']
